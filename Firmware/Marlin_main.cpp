      ...
      break;
        ...
        case 941: // M941: Send Input Event
            if (code_seen('W')) // Encoder change, signed integer
            {
                ...
                change_encoder(1, ENCODER_PULSES_PER_STEP);
            }
            ...
            if (code_seen('N')) // Button long press
            {
                //trigger long button press
                long_press_button();
            }
            if (code_seen('U')) // Up button
                {
                    //trigger up button press
                    change_encoder(-1, ENCODER_PULSES_PER_STEP);
                }
            if (code_seen('D')) // Down button
                {
                    //trigger down button press
                    change_encoder(1, ENCODER_PULSES_PER_STEP);
                }
            if (code_seen('L')) // Left button
                {

//                    press_button();

                }
            if (code_seen('R')) // Right button
                {
                press_button();
                }
